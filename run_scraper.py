...
MAX_PER_RUN = 300     # max companies per scrape run (rolling window)
MANUAL_BATCH_SIZE = 300  # companies per manual run (ordered by oldest last_scraped)
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        batch_size = MANUAL_BATCH_SIZE if is_manual else MAX_PER_RUN
