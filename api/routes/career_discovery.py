...
async def _search_sonar(company_name: str, website_url: Optional[str]) -> ProviderResult:
    ...
    prompt = (
        f"Your task is to find the exact URL of the page where \"{company_name}\"{site_hint} lists ALL their open job positions.\n\n"
        "TARGET: A page that directly renders a list or board of individual job openings — every current vacancy visible at once.\n\n"
        "PREFERRED SOURCES (in order):\n"
        "1. A known ATS platform board for this company:\n"
        ...
        "- A blog post, news article, or press release\n"
        "- Any URL that does not itself display a list of individual job postings\n"
        "- Any URL you are not certain exists — do NOT invent or guess URLs by appending random words to a domain\n\n"
        "RESPONSE FORMAT:\n"
        "- Return ONLY the full URL, starting with https://\n"
        "- No explanation, no punctuation, no extra text\n"
        "- If no qualifying page exists or you are not certain the URL is real, return exactly: NOT_FOUND"
    )
