"""
Migration v4 — one-time data cleanup (idempotent).

1. Deletes 12 invalid/acquired/closed companies and their jobs.
2. Sets website_url for ~120 companies that were missing it.

Safe to run multiple times: deletions are no-ops if already gone,
URL updates use UPDATE … WHERE name = X (no harm if already set).

Usage:  python migrate_v4_cleanup.py
"""
import asyncio
import os
    ...
    async with engine.begin() as conn:
        ...
        for name in COMPANIES_TO_DELETE:
            ...
            if result is None:
                continue
            cid = result[0]
            await conn.execute(text("DELETE FROM jobs WHERE company_id = :id"), {"id": cid})
            await conn.execute(text("DELETE FROM companies WHERE id = :id"), {"id": cid})
            deleted += 1
            print(f"  [DELETED] {name}")

        # ── 2. Website URL updates ─────────────────────────────────────────────
        for name, url in WEBSITE_UPDATES.items():
            r = await conn.execute(
                text("UPDATE companies SET website_url = :url WHERE name = :name AND (website_url IS NULL OR website_url = '')"),
                {"url": url, "name": name},
            )
            if r.rowcount:
                updated += 1

    print(f"\nMigration v4 complete: {deleted} deleted, {updated} website URLs set.")
